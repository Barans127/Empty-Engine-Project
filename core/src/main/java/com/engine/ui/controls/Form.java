...
public abstract class Form implements Window {
    private ControlHost v; // default controller.
//    private ArrayList<PopUp> exist; // laiko pop up. kad nedingtu.
    ...
    public Form(ControlHost v){
        if (v==null)
            this.v = new ControlHost();
        else
            this.v = v;
        this.v.setForm(this);
//        exist = new ArrayList<>();
...
//            System.out.println("Form already has defined template. Remove old template before adding new one.");
    ...
    public void removeFormsTemplate(boolean destroyPhysicsBodies){
        if (formsTemplate != null){
            for (Control e : formsTemplate.getInterfaces()){
                removeControl(e);
            }
...
//    }
    ...
    public boolean addControl(Control a) {
    ...
    public boolean addControl(Control e, int index){
...
//		mouseMoved();
    ...
    public ControlHost getController() {
        return v;
    }
...
//	}
    ...
    /** This will be called after default {@link ControlHost} events if they return false (no buttons reactions etc).
    ...
    /** @return the most top item which prevents form from inputs (Inputs are handled in {@link ControlHost} and not form).
